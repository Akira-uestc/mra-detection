...
from freq_reconstruction import (
    FrequencyOnlyReconstructor,
    reconstruct_sequence as reconstruct_freq_sequence,
    train_model as train_freq_model,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="单频域模块消融：使用频域重构，并接入 mra.py 的异常检测流程。"
    )
    parser.add_argument(
        "--train-glob",
    ...
    parser.add_argument(
        "--output-dir",
        default="outputs/freq_transformer_detection",
        help="输出目录，默认: outputs/freq_transformer_detection",
    )
    parser.add_argument(
        "--freq-seq-len",
        type=int,
...
def main() -> None:
    ...
    train_windows, train_window_masks = build_windows(
        train_scaled,
        train_mask.astype(np.float32),
        seq_len=args.freq_seq_len,
        stride=args.stride,
    )
    test_windows, test_window_masks = build_windows(
        test_scaled,
    ...
    freq_model = FrequencyOnlyReconstructor(
    ...
    print("\n重构 train/test 序列...")
    train_reconstructed_scaled = reconstruct_freq_sequence(
        model=freq_model,
    ...
    test_reconstructed_scaled = reconstruct_freq_sequence(
        model=freq_model,
