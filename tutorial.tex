...
%	\uestccover[<学院名称排版风格>]{<论文题目>}{<学科专业>}{<学号>}{<作者姓名>}{<指导教师>}{<教师职称>}{<学院>} % 单学位学士、研究生通用
    ...
    AGF-ADNet 采用插补与重构联合建模的总体思路。多采样率异常检测面临三个相互耦合的关键问题：缺失位置削弱时序连续性，变量之间存在非平稳耦合关系，不同采样频率下的周期结构和相位信息难以通过统一的时域模型充分表达。若仅将缺失值恢复视为独立的数据清洗步骤，插补结果往往只追求点值误差最小，无法兼顾后续异常判别所需的结构信息；若直接在缺失序列上执行异常检测，模型又容易将采样率差异和缺失模式混同为异常信号。因此，AGF-ADNet 将缺失恢复、结构建模和异常判别纳入同一优化框架，使插补结果既要接近真实观测，也要有利于后续全局重构和异常区分。
    
    在模块构成上，模型先通过双专家协同插补模块恢复缺失位置以得到完整窗口，然后引入采样率感知 Transformer 对完整窗口进行全局重构，最后结合重构偏差、专家分歧和分布漂移构建异常分数。这样的分层设计具有明确的功能分工：前一层负责恢复完整、稳定且具有结构一致性的输入表示，后一层负责从完整窗口中提取正常工况的全局演化规律。模型的整体流程可概括为
    \begin{align}
    ...
    \begin{table}[htbp]
        ...
        \begin{tabular}{p{2.4cm} p{4.8cm} p{3.0cm} p{3.0cm}}
            ...
            模块 & 主要功能 & 输入 & 输出 \\
            \hline
            数据预处理 & 完成缺失掩码生成、观测值标准化、窗口化与采样率模式推断、数据分块 & 原始序列 $\mathbf{D}$ & $\mathbf{X}$、$\mathbf{M}$、$\mathbf{r}$、$\mathbf{s}$ \\
    ...
    同时，基于上述推断得到的采样率组，可以对数据进行分块（chunking），并将相似采样率组的数据排列在一起，这一步的意义在于：相似采样率的变量通常具有较强的相关性，有助于充分发掘数据潜在特征。数据分块示意如图\ref{fig:data_chunking}所示。其中红色代表缺失样本。
    ...
    \begin{equation}
        \mathbf{X}^{\mathrm{in}} = \mathbf{X} \odot (1-\widetilde{\mathbf{M}}).
    \end{equation}
    这种自监督构造使模型可以在具有真实值的位置上学习缺失恢复能力，同时保持结构性缺失场景下的训练一致性。进一步地，掩码机制还迫使模型在不同缺失模式下学习稳定的恢复映射，从而降低模型对固定缺失分布的过拟合风险。由于训练阶段的人为遮挡仅发生在原本可观测的位置，插补误差始终可以与真实值对齐计算，因此训练目标具有明确的统计意义。
...
%     \hfill
    ...
    缺失值恢复之后，仍需要从完整窗口中判断样本是否偏离正常运行规律。为此，AGF-ADNet 在插补模块之后引入采样率感知 Transformer 重构检测器。该检测器面向完整窗口 $\mathbf{X}^{\mathrm{comp}}$ 学习正常模式下的全局演化规律，再以重构偏差刻画异常程度。将异常检测置于插补之后的考虑是，多采样率序列在统一时间轴上存在结构性空缺，若直接对不完整输入建模，检测器需要同时学习缺失模式、采样频率差异和正常动态规律，任务耦合过强。通过先获得较为完整的窗口表示，检测器可以更集中地刻画系统在正常工况下的全局时序结构。
    ...
    在当前实验设置中，检测器采用 3 层 Transformer 编码器，时间 Token 维度为 128，多头注意力头数为 4。Transformer 可以直接在整个窗口范围内建模任意两个时间位置之间的依赖关系，因此不仅能够感知局部突变，也能够刻画慢变漂移、周期累积误差以及跨阶段状态转移。采用残差重构头有利于让网络重点学习相对于输入完整窗口的修正量，提高训练稳定性。
    ...
    本章围绕 AGF-ADNet 的结构与机理，对数据预处理、采样率元数据构造、双向预插补、自适应图学习、图重构专家、频域重构专家、采样率感知门控融合、Transformer 全局重构检测器以及异常评分机制进行了系统阐述。与传统先插补后检测的串联式方法相比，AGF-ADNet 通过联合优化实现了插补与检测的协同建模；同时，模型显式引入采样率类别和相位信息，使其能够更充分地适应多采样率工业过程中的结构性缺失、变量耦合以及周期动态特征。
  ...
  \end{figure}
    ...
    因此，在当前证据下，建议继续采用默认配置 \(\texttt{seq\_len}=50\)、\(\texttt{holdout\_ratio}=0.15\)、\(\texttt{detector\_d\_model}=128\)、\(\texttt{detector\_layers}=3\)、\(\texttt{gcn\_hidden\_dim}=32\) 作为后续主实验设置。
        ...
        本文围绕电源系统多采样率过程异常检测问题开展研究，针对多源监测变量采样频率不一致所引起的结构性缺失、时间异步、变量耦合关系难以表达以及周期频谱信息利用不足等关键挑战，提出了一种面向多采样率电源系统的自适应图--频率异常检测网络 AGF-ADNet。论文以三相全桥逆变系统为应用背景，从问题建模、模型设计、实验验证和机制分析等方面进行了较为系统的研究，主要工作与结论可概括如下。

        第一，围绕多采样率工业数据的基本特征，构建了异常检测问题描述与数据处理流程。本文通过缺失掩码生成、仅基于观测位置的标准化、滑动窗口构造以及采样率元数据推断等步骤，将原始多采样率监测序列转化为适合统一建模的窗口样本表示，为后续插补与检测协同优化奠定了数据基础。该表示方式能够显式保留何处缺失、何处观测的结构信息，同时为采样率感知建模提供必要先验。

        第二，针对多采样率场景下缺失恢复不可靠与异常表征不足的问题，设计了 AGF-ADNet 模型。模型在插补阶段引入双向预插补与自监督再遮挡机制，为学习型缺失恢复提供稳定初始化与有效监督；进一步构建自适应图重构专家与频域重构专家，分别从跨变量依赖关系和周期频谱结构两条路径补充信息，并通过采样率感知门控融合模块生成最终插补结果。在检测阶段，模型利用采样率感知 Transformer 对完整窗口进行全局重构，同时融合重构偏差、双专家分歧与分布漂移三类信号构建异常评分，实现了插补与检测的一体化协同建模。
        ...
        第四，结合消融实验与参数敏感性实验，对模型有效性来源进行了进一步分析。消融结果表明，完整 AGF-ADNet 在所有消融设置中取得最优性能，说明双专家插补、门控融合、采样率感知检测器以及多源异常评分之间存在明显协同作用。图频门控 + Transformer 是最接近完整模型的消融版本，F1-Score 达到 0.9250，但与完整模型相比仍存在召回率差距，表明采样率感知建模、相位信息和分布漂移分数对故障窗口覆盖能力具有进一步增益。单专家结果显示，仅图专家 + Transformer 明显优于仅频域专家 + Transformer，说明当前故障场景中变量间耦合关系是更主要的判别信息来源；而图频门控 + 重构输出和两个单专家直接重构输出的结果则表明，仅依赖插补重构误差难以形成稳定连续的异常响应，Transformer 检测器对于最终检测性能具有关键作用。参数敏感性结果表明，序列窗口长度是最关键的敏感因素，过短窗口会显著削弱异常片段的连续检出能力；相比之下，再遮挡比例、检测器宽度与层数在当前取值范围内表现出较好的鲁棒性，GCN 分支隐藏维度则呈现较明显的中间最优特征。

        综合来看，本文所提出的 AGF-ADNet 能够较好地缓解多采样率场景下的结构性缺失问题，并充分利用变量耦合关系、频谱特征与全局时序演化信息，在电源系统异常检测任务中表现出较高的检测精度、较强的鲁棒性以及一定的工程应用潜力。上述研究结果表明，面向多采样率工业过程的“插补--检测协同优化”建模思路是可行且有效的。
        ...
        第二，面向在线监测与边缘部署需求，进一步优化模型效率。当前模型采用双专家插补与 Transformer 全局重构相结合的结构，在实时性、模型规模和部署成本方面仍有优化空间。后续可探索构建更轻量话，适用于在线诊断、嵌入式或边缘设备的多采样率异常检测框架。

        第三，增强模型对复杂缺失模式与不确定性的适应能力。本文重点关注多采样率带来的结构缺失，实际工业现场可能同时存在随机丢包、通信异常、标签噪声以及多故障耦合等问题。未来可进一步增强系统在复杂场景下的鲁棒性。

        总体而言，随着工业现场多源异构感知系统的不断普及，多采样率过程异常检测将面临更复杂的数据形态与更高的工程部署要求。围绕真实场景数、在线据轻量建模、系统鲁棒性等方向持续深入，有望进一步提升相关方法的理论价值与应用价值。
