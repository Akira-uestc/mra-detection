...
\end{align}
其中 $\bm{X}^{\text{filled}}$ 由“观测位置保留输入值、缺失位置使用插补值”的标准插补公式构造，后文会给出其精确表达式。
...
融合后的插补序列 $\bm{X}^{\text{filled}}$ 送入 Transformer。代码中的投影为：
\begin{equation}
...
\section{关于 \texorpdfstring{$\bm{X}^{\text{filled}}$}{Xfilled} 的实现}
...
\subsection{与原始窗口的等价表达}

由于进入模型前已经做过
\begin{equation}
    \bm{X}^{\text{in}} = \bm{X} \odot (1-\bm{M}),
\end{equation}
所以 $\bm{X}^{\text{filled}}$ 也可以等价写成
\begin{equation}
    \bm{X}^{\text{filled}}
    =
    \bm{X} \odot (1-\bm{M})
...
这表示：
\begin{itemize}
    \item 被标记为 observed 的位置，Transformer 看到原始输入值；
    \item 被标记为 missing 的位置，Transformer 看到门控融合得到的插补值。
\end{itemize}

\paragraph{实现意义}
这是一个标准的插补式重构接口：
\begin{equation}
    \text{观测位置保真} + \text{缺失位置补全}.
\end{equation}
因此 Transformer 的输入同时保留了原始观测信息和模型生成的插补信息。
...
\begin{enumerate}
    ...
    \item \textbf{当前 Transformer 输入采用标准插补形式。} 观测位置保留输入值，缺失位置使用 $\bm{X}^{\text{imp}}$ 补全。
...
但在实际部署或复现实验时，仍应特别注意本文档指出的实现细节，尤其是掩码语义、阈值公式以及 \texttt{X\_filled} 的构造方式。
